...
void create_frame(tFrameStack *st, string *name_class, tNode *node, int *arg_count)
{
    //printf("\n---------create----------");
...
void destroy_frame(tFrameStack *st)
{
    ...
    while (help_list->First != NULL)
    {
        ...
        free_pointer(help_list->First, true);
        // nastavi novou
        help_list->First = help_var;
    }
    // zrusime seznam
    free_pointer(help_list, true);
    st->top--;
...
tData *get_value(tFrameStack *st, string *name, bool init)
{
    ...
    if (help->Act == NULL)
    {
        ...
        for (int i = st->top; i > -1; i--)
        {
            if (st->stack[i]->type == CLASS &&
                equal_str(st->stack[i]->name_frame->str, help->name_par->str))
            {
                help = st->stack[i];
                break;
            }
