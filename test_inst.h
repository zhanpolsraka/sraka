...
/* */               INSTR_ASS_ARG, // addr3 - указатель на tData аргумента функции
                    INSTR_INSERT,  // addr1 - указатель на tData значения для помещения его в стэк

                    INSTR_BEG_COND, // начало условия для if и while
/* 17 = 'if'      */INSTR_IF, // addr1 - индекс положки конца if, addr2 - индекс началаположки else (может не быть), addr3 - индекс конца положки else (может не быть)
/* 18 = 'else'    */INSTR_ELSE,
