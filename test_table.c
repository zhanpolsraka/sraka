...
void destroy_tree(tNode *begin){
	...
	switch(begin->type){
		...
		case FUNCTION:
			if(begin->variables != NULL) destroy_tree(begin->variables);
			break;

		case CLASS:
			if(begin->functions != NULL) destroy_tree(begin->functions);
			if(begin->variables != NULL) destroy_tree(begin->variables);
			break;
	}
	if(begin->l_node != NULL) destroy_tree(begin->l_node);
	if(begin->r_node != NULL) destroy_tree(begin->r_node);
	strFree(&begin->key);
	free(begin);
}
...
tNode *get_node(string *key, NodeType type, tNode *begin){
	...
	else if(begin == NULL) begin = tree->root;
	if(begin != NULL){
		// porovname klice
...
bool add_node(tNode *new, tNode *begin){
	...
	// jdeme do leveho podstromu
	if(cmp < 0){
		if(begin->l_node == NULL){				// udelame novy prvek levym podstromem
			begin->l_node = new;
			if(new->type == CLASS) tree->active_class = new;	// udelame ho aktivnim
			else if(new->type == FUNCTION) tree->active_func = new;
		}
		else if(!add_node(new, begin->l_node))	// nebo jdeme dal po levemy podstromu
			return false;
	}
	// jdeme do praveho podstromu
	else if(cmp > 0){
		if(begin->r_node == NULL){				// udelame novy prvek pravym podstromem
			begin->r_node = new;
			if(new->type == CLASS) tree->active_class = new;	// udelame ho aktivnim
			else if(new->type == FUNCTION) tree->active_func = new;
		}
		else if(!add_node(new, begin->r_node))	// nebo jdeme dal po pravemu podstromu
			return false;
	}
	else{
		throw_err(PR_ERROR, TREE_ERROR);
...
bool create_node(string *id, NodeType type, int v_type, bool stat){
	...
	else if(!add_node(new, begin)) return false;
	return true;
}
...
bool create_arg(string *id, int type){
	...
	else if(!add_node(arg, tree->active_func->variables))
		return false;
