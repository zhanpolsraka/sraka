...
void execute(tInstrStack *st)
{
    ...
    while (top > -1)
    {
        ...
        if (st->inst[top]->type == INST_CLASS)
        {
            ...
            create_frame(frames, get_node(st->inst[top]->name, CLASS, NULL), NULL, NULL);
    ...
    while (top > -1)
    {
            ...
            equal_str(st->inst[top]->name->str, "run"))
        {
            ...
            create_frame(frames, run_node, "Main",NULL);
...
void exec_instructions(tInstrStack *st, tExprStack *exp_st, int indx)
{
    ...
    while (true)
    {
        ...
        * addr1 - the node of the function in binary tree
        * addr2 - pointer to a function name
        */
    ...
    */
    else if (operation == ASSIGNMENT || operation == INSTR_ASS_ARG)
    {
        ...
        if (operation == ASSIGNMENT)
            /* simultaneously initialize its */
            destination = get_value(frames, instr->addr3, true);
        else
